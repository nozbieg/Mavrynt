...
/**
 * Typed locale hook. Mirrors the `useLocale` hook from `mavrynt-landing`
...
export const useLocale = (): LocaleController => {
  ...
  // Keep `<html lang>` synced with the active locale — covers initial
  // mount + subsequent changes. WCAG 3.1.1 / 3.1.2.
  useEffect(() => {
